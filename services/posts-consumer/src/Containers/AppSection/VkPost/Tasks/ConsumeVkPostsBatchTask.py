"""
ConsumeVkPostsBatchTask: Чтение батча VK сообщений из Kafka.
...
async def consume_vk_posts_batch_task(
    consumer: AIOKafkaConsumer,
    batch_size: int = 100,
    timeout_ms: int = 10000
) -> List[Dict[str, Any]]:
    """
    Task: Consume батч VK постов из Kafka.
    ...
    Returns:
        Список VK сообщений (parsed from JSON)
    ...
    for tp, msgs in result.items():
        for msg in msgs:
            try:
                ...
                logger.warning(f"Failed to parse VK JSON message: {exc}")
                continue
            except Exception as exc:
                logger.warning(f"Failed to process VK message: {exc}")
                continue
        
        if len(messages) >= batch_size:
            break
    
    logger.debug(f"Consumed {len(messages)} VK messages from Kafka")
